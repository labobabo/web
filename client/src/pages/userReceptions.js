...
const DoctorReceptions = () => {
    ...
    const handleGenerateConclusion = () => {
        // Ваша логика для генерации заключения
    };
    ...
    return (
        <Container>
                            ...
                            <Col className="d-flex justify-content-end align-items-center">
                                {isPastDateTime(`${reception.date} ${reception.time}`) ? (
                                    <>
                                        ...
                                        {!reception.evaluated && (
                                            <Button variant="success" className="btn-purple btn-size" onClick={() => handleRateDoctor(reception)}>Оценить врача</Button>
                                        )}
                                        {reception.rating ? (
                                            <Button variant="info" className="me-2 btn-size" disabled>Врач оценен</Button>
                                        ) : null}
                                        <Button variant="secondary" className="btn-purple btn-size" onClick={handleGenerateConclusion}>Сформировать заключение</Button>
                                    </>
                                ) : (
                                    <Button variant="danger" className="btn-purple btn-size" onClick={() => handleCancelReception(reception.receptionId)}>Отменить запись</Button>
                                )}
                            </Col>
                        </Row>
                    </ListGroup.Item>
            ...
            <Modal show={showDetails} onHide={handleCloseDetails} centered>
                ...
                <Modal.Body>
                    <p className="mb-2">Дата осмотра: {selectedReception && selectedReception.receptionDate}</p>
